...
class CI_Security {
	...
	public function csrf_verify()
	{
		...
		{
			if (in_array($this->CI->uri->uri_string(), $exclude_uris))
			{
				return $this;
			}
		}
		...
		{
			throw new CI_Show_error('The action you have requested is not allowed.');
		}
	...
	{
		throw new CI_ShowEerror('The action you have requested is not allowed.');
	}
	...
	public function xss_clean($str, $is_image = FALSE)
	{
		...
		// This permits our tests below to work reliably. We only convert entities
		...
		if ($is_image === TRUE)
		{
			// Images have a tendency to have the PHP short opening and closing tags
			// every so often so we skip those and only do the long opening tags.
		...
		$words = array('javascript', 'expression', 'vbscript', 'script', 'applet', 'alert', 'document', 'write',
			'cookie', 'window');
		...
		foreach ($words as $word)
		{
			...
			$str = preg_replace_callback('#('.substr($temp, 0, -3).')(\W)#is', array($this, '_compact_exploded_words'),
				$str);
		}
		...
		// Similar to above, only instead of looking for tags it looks for PHP
		// and JavaScript commands that are disallowed. Rather than removing
		// the code, it simply converts the parenthesis to entities rendering
		// the code un-executable.
		// For example:	eval('some code')
		// Becomes:		eval&#40;'some code'&#41;
		$str = preg_replace('#(alert|cmd|passthru|eval|exec|expression|system|fopen|fsockopen|file|'.
			'file_get_contents|readfile|unlink)(\s*)\((.*?)\)#si', '\\1\\2&#40;\\3&#41;', $str);
		...
		// is done whether any unwanted, likely XSS, code was found. If not, we
		// return TRUE, as the image is clean. However, if the string post-conversion
		// does not matched the string post-removal of XSS, then it fails, as there
		// was unwanted XSS code found and removed/changed during processing.
		if ($is_image === TRUE)
		{
			return ($str == $converted_string) ? TRUE: FALSE;
		}

		$this->CI->log_message('debug', 'XSS Filtering completed');
		return $str;
	}
	...
	{
		return $this->entity_decode($match[0], strtoupper($this->CI->config->item('charset')));
	}
	...
	protected function _remove_evil_attributes($str, $is_image)
	{
		...
		do
		{
			$str = preg_replace('#<(/?[^><]+?)([^A-Za-z\-])('.implode('|', $evil_attributes).
